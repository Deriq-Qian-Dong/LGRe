...
def get_passages(text, plen=150, overlap=75):
    words = text
    s, e = 0, 0
    passages = []
    while s < len(words):
        ...
        s = s + plen - overlap
    if len(passages) > 6:
        passages = [passages[0]] + random.sample(passages[1:-1], 4) + [passages[-1]]
    ret = []
    for p in passages:
        ret += p
    return ret
    ...
    while True:
        ...
        for qid in qids:
            ...
            if neg_doc is None:
                ...
                continue
            yield qid, pos_id, query_tok, get_passages(model.tokenize(pos_doc))
            yield qid, neg_id, query_tok, get_passages(model.tokenize(neg_doc))
...
def _iter_valid_records(model, dataset, run):
    ...
    for qid in run:
        ...
        for did in run[qid]:
            ...
            yield qid, did, query_tok, get_passages(doc_tok)
